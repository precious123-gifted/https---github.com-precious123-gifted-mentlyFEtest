...
interface SvgComponentProps {
  ...
  [key: string]: any; // To handle other props passed to SVG
}

const SvgComponent = ({ className, fill, ...props }: SvgComponentProps) => {
  return (
    <svg
      ...
      width={40} 
      height={40}  
      fill={fill || 'none'} // Default to 'none' if no fill is provided
      className={className} // Apply the className to the SVG element
      {...props} // Spread the rest of the props onto the SVG
    >
      <path
      ...
      />
      <path fill={fill || 'currentColor'} d="M13.636 61.111h18.182v5.556H13.636V61.11Z" />
      <path
        fill={fill || 'currentColor'}
        fillRule="evenodd"
